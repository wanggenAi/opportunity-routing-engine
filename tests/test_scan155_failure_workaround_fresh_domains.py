...
class Scan155FailureWorkaroundFreshDomainTests(unittest.TestCase):
    ...
    def test_machine_state_finalizes_scan154_and_advances(self):
        ...
        self.assertGreaterEqual(int(state["last_completed_scan_id"].rsplit("_",1)[1]),155)
        self.assertGreaterEqual(int(state["next_scan_id"].rsplit("_",1)[1]),156)
        self.assertEqual(state["scan155_failure_workaround_fresh_domains"]["final_jev_next_action"],"ADVANCE_TO_NEXT_SCAN")
