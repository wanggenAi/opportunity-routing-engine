...
SCAN_PATH = ROOT / "data" / "research_runs" / "attraction_scan_145.json"
...
def test_scan145_passes_strategic_drift_guard():
    scan=load(SCAN_PATH)
    assert scan["scan_id"]=="ATTRACTION_SCAN_145"
    assert strategic_drift_errors(scan)==[]
    assert scan["drift_audit"]["scan144_industrial_aftermarket_subproblem_not_used_as_prior"] is True
    assert scan["drift_audit"]["incumbent_presence_evaluated_before_retention"] is True

def test_scan145_has_no_high_attraction_beacon():
    scan=load(SCAN_PATH)
...
def test_machine_state_closes_all_scan145_formations_without_promotion():
    state=load(STATE_PATH)
    assert state["last_completed_scan_id"]=="ATTRACTION_SCAN_145"
    assert state["next_scan_id"]=="ATTRACTION_SCAN_146"
    resolved={row["formation_id"]:row["verdict"] for row in state["resolved_research_formations"]}
    for suffix in ("F1","F2","F3","F4","F5"):
        assert resolved[f"ATTRACTION_SCAN_145-{suffix}"].startswith("DEMOTED_")
