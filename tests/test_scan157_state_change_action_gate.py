...
class Scan157StateChangeActionGateTests(unittest.TestCase):
    ...
    def test_every_formation_records_state_change_and_gate_owner(self):
        scan=load(SCAN_PATH)
        for row in scan["examined_formations"]:
            gate=row["state_change_gate"]
            for key in ("event_trace","affected_actor_population","counterparty_population","decisive_action_gate"):
                self.assertEqual(gate[key]["state"],"EVIDENCED")
                self.assertTrue(gate[key]["evidence_refs"])
            self.assertIn(gate["decisive_action_gate"]["owner_state"],{"INCUMBENT_OWNED","REGULATOR_PLATFORM_OWNED","REGULATOR_OWNED"})

    def test_visible_event_does_not_fake_operator_control(self):
        ...
        self.assertTrue(scan["drift_audit"]["public_event_visibility_not_equated_with_operator_control"])

    def test_food_residual_pressure_is_preserved_but_expert_bound(self):
        scan=load(SCAN_PATH); row={x["formation_id"]:x for x in scan["examined_formations"]}["ATTRACTION_SCAN_157-F3"]
    ...
    def test_all_three_are_authoritatively_resolved(self):
        state=load(STATE_PATH); resolved={x["formation_id"]:x["verdict"] for x in state["resolved_research_formations"]}
        for suffix in ("F1","F2","F3"): self.assertTrue(resolved[f"ATTRACTION_SCAN_157-{suffix}"].startswith("DEMOTED_"))
        self.assertEqual(len(state["scan157_state_change_action_gate"]["authoritative_closures"]),3)
