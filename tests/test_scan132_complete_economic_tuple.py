...
def test_scan132_remains_closed_after_scan133_becomes_current_auto_input():
    state = load(STATE)
    path = resolve_scan_path("auto", state, research_dir=ROOT / "data" / "research_runs")
    assert state["last_completed_scan_id"] == "ATTRACTION_SCAN_133"
    assert path == ROOT / "data" / "research_runs" / "attraction_scan_133.json"
    scan = load(SCAN)
...
def test_scan132_evidence_pack_matches_zero_retention_machine_truth():
    ...
    assert state["next_scan_id"] == "ATTRACTION_SCAN_134"
